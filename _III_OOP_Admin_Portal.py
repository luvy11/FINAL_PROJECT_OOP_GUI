...
# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    welcome_label = my_canvas.create_text(620, 361, text="Welcome to the Admin Portal", font=("Century Gothic", 45), fill="white")
...
# ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def switch_to_add_profile_portal(): # For Add Profile Button
        window.destroy()
        import _I_OOP_Information_Portal
        _I_OOP_Information_Portal.main(username)
    # Adding the needed buttons
    log_out_image = ImageTk.PhotoImage(Image.open("Log_Out_Logo.png").resize((39, 32)))
    log_out_button = gui.button_design(header_frame, 1690, 0, 210, 58, "#065494", "green", " Log Out", ("Century Gothic", 21, 'bold'))
    ...
    add_profile_image = ImageTk.PhotoImage(Image.open("Add_Profile_Logo.png").resize((56, 61)))
    add_profile_button = gui.button_design(main_frame, 600, 519, 495, 104, "#065494", "white", " ADD PROFILE",("Century Gothic", 30, 'bold'))
    add_profile_button.config(image=add_profile_image, compound=tk.LEFT, relief=tk.FLAT, activebackground="#065494", command=switch_to_add_profile_portal)
